...
/**
 * POST /api/users/song-reactions/:setlistSongId
 * React with 🔥 to a specific setlist song performance
 * Requires authentication
 */
router.post('/song-reactions/:setlistSongId', async (req, res) => {
    try {
        const { setlistSongId } = req.params;
        const authHeader = req.headers.authorization;
        ...
        const { error } = await supabase
            ...
            .insert([{ setlist_song_id: setlistSongId, user_id: user.id }]);
...
/**
 * DELETE /api/users/song-reactions/:setlistSongId
 * Remove your own reaction from a setlist song
 * Requires authentication
 */
router.delete('/song-reactions/:setlistSongId', async (req, res) => {
    try {
        const { setlistSongId } = req.params;
        const authHeader = req.headers.authorization;
