...
/**
 * GET /api/users/community-map
 * Every distinct show that at least one signed-up member has marked as attended,
 * with venue info and how many members attended it. Aggregate/anonymous — no
 * per-user attendance is exposed, just which shows the community as a whole has
 * been to. Public — no authentication required.
 */
router.get('/community-map', async (req, res) => {
    try {
        // Tally attendee counts per show, paginated since user_shows can exceed
        // PostgREST's 1000-row default as the community grows.
        ...
        for (let rangeStart = 0; ;) {
            const { data: page, error } = await supabase
                ...
                .range(rangeStart, rangeStart + 999);
            if (error) throw error;
            (page || []).forEach(({ show_id }) => {
                attendeeCounts[show_id] = (attendeeCounts[show_id] || 0) + 1;
            });
            if (!page || page.length < 1000) break;
            rangeStart += 1000;
        }

        const showIds = Object.keys(attendeeCounts);
        if (showIds.length === 0) return res.json({ shows: [] });
        ...
        const result = shows
            .map(s => ({
                ...
                venues: s.venues || null,
                attendeeCount: attendeeCounts[s.id] || 0,
            }))
        ...
        res.status(500).json({ error: 'Failed to fetch community show map data' });
    }
});
